...
fun Context.retrieveAllContacts(
    ...
    contentResolver.query(
        ...
        if (it.moveToFirst()) {
            do {
                val contactId = it.getLong(it.getColumnIndex(CONTACT_PROJECTION[0]) ?: 0)
                val name = it.getString(it.getColumnIndex(CONTACT_PROJECTION[2]) ?: 0) ?: ""
                val hasPhoneNumber =
                    it.getString(it.getColumnIndex(CONTACT_PROJECTION[3]) ?: 0).toInt()
